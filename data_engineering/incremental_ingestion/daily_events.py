...
client = OpenAI(
  ...
  api_key="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",
)
web_url = "https://www.thehindu.com/latest-news/?page="
db_config = {
        "dbname":"borderdata",
        "user":"postgres",
        "password":"Post#6789#bandu546",
        "host":"border-threat-db.cd0aywmyspsn.us-east-1.rds.amazonaws.com",
        "port":5432
    }
    ...
    try:
        ...
        with engine.begin() as conn:
            ...
            for row in final_data:
                i+=1
                # row['event_date'] = datetime.strptime(row['event_date'], "%Y-%m-%d").date()  # Convert date
                conn.execute(
                    text("""
                        INSERT INTO defence_data (
                            source_url,source,source_scale,notes,event_type,event_date,country,actor1,actor2,admin1,
                            admin2,location,fatalities,civilian_targeting,latitude,longitude,weather_condition,
                            max_temp,min_temp
                        ) VALUES (
                            :source_url, :source, :source_scale, :notes, :event_type, :event_date, :country,
                            :actor1, :actor2, :admin1, :admin2, :location, :fatalities, :civilian_targeting,
                            :latitude, :longitude, :weather_condition, :max_temp, :min_temp
                        )
                    """),
                    row
                )
